...
const nameToCategory = (name: string): Technology["category"] | undefined => {
	...
	if (/(mysql|postgres|postgresql|mongodb|redis|prisma)/.test(n))
		return "database";
	if (/(aws|gcp|azure|vercel|netlify)/.test(n)) return "cloud";
	if (/(docker|git|zod|zustand|expo|sanctum|spatie|resend|bun)/.test(n))
		return "tool";
	return undefined;
};
	...
	return (
		<div className={cn("flex flex-wrap gap-2", className)}>
			{list.map((item, idx) => {
				...
				const category =
					isObj && item.category ? item.category : nameToCategory(name);
				const base =
