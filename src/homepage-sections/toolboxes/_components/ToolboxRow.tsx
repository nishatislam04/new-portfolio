				...
				{category.items.map((item, itemIndex) => {
					...
					return (
						<motion.div
							...
							whileTap={{ scale: 0.95 }}
							className={cn(
								"flex flex-col items-center gap-3 py-2 px-4 rounded-xl cursor-pointer",
						...
						>
							...
							<div className="transition-all duration-300 group-hover:scale-110">
								{IconComponent ? (
									isRaster ? (
										<Image
											src={IconComponent as any}
											alt={item.title}
											width={40}
											height={40}
											className={cn("size-10 object-contain select-none")}
											priority={false}
											unoptimized
											decoding="async"
											loading="lazy"
										...
										(() => {
											const SvgComp = IconComponent as React.ComponentType<
												React.SVGProps<SVGSVGElement>
											>;
											return (
												<SvgComp
													className={cn(
														"size-10 transition-transform duration-300 select-none",
													)}
												/>
											);
										})()
